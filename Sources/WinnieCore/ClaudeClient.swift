...
public struct ClaudeClient: Sendable {
    ...
    private static let mailNote = """
        ...
        Когда Серёжа просит проверить почту, посмотри непрочитанные во входящих \
        (list_emails с запросом «is:unread in:inbox») и дай сводку, а не перечень: сначала \
        письма от живых людей и всё, что ждёт его ответа или действия — по строке на письмо, кто \
        и чего хочет; затем одной строкой остальное (рассылки, уведомления, чеки) — просто \
        сколько их и от кого. Письмо открывай через read_email, только если по теме и отрывку \
        непонятно, чего от него хотят, и не больше трёх за раз. Если непрочитанных нет, так и скажи.
        """
