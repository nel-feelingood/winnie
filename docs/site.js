...
if (list) {
  const cases = [
    { c: '--violet', title: 'Слушать по шорткату', note: 'Диктовка из любого приложения. Распознавание на устройстве, ответ можно получить вслух.',
      keys: ['⇧', '⌘', 'E'], pose: 'listening', ask: 'сколько граммов в стакане муки?', text: 'Около 130 граммов.' },
    { c: '--acc', title: 'Напомнить', note: 'Время берётся из фразы. Уведомление приходит, даже когда медведь спрятан.',
      ask: 'напомни через 40 минут достать пирог', card: ['Достать пирог', 'сегодня, 14:40'] },
    { c: '--blue', title: 'Быстрая команда', note: 'Кнопка в пустом чате. Подпись короткая, инструкция под ней любая.',
    ...
    { c: '--yellow', title: 'Записать', note: 'Заметки – Markdown-файлы на диске. Медведь создаёт и дополняет их сам.',
      ask: 'запиши в Идеи: прогулка по экрану', text: 'Добавил в заметку «Идеи».' },
    { c: '--blue', title: 'Перевести', note: 'Текст в ответе можно выделить, рядом появляется кнопка Copy.',
      ask: 'переведи вежливо: давайте перенесём созвон на четверг', text: 'Could we please move the call to Thursday?' },
    { c: '--blue', title: 'Найти', note: 'Ищет в вебе, под ответом ссылки на источники.',
      ask: 'до скольки сегодня работает мэрия?', text: 'До 18:00, приём документов до 17:30.', source: '1. Сайт мэрии' },
    { c: '--green', title: 'Разобрать почту', note: 'Gmail, доступ только на чтение.',
  ...
  cases.forEach((item, index) => {
    ...
    button.innerHTML = `<b>${item.title}</b><small>${item.note}</small>`;
